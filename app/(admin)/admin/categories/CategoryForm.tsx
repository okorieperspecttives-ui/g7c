...
export default function CategoryForm({ initialData }: CategoryFormProps) {
  ...
  const displayImageUrl = previewUrl?.startsWith("blob:") 
    ? previewUrl 
    : (previewUrl ? (previewUrl.startsWith("http") ? previewUrl : getPublicUrl("category-images", previewUrl)) : null);

  return (
    <form action={handleSubmit} className="space-y-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        <Link href="/admin/categories" className="p-2 rounded-xl bg-secondary hover:bg-secondary/80 transition-colors">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <h1 className="text-3xl font-black tracking-tight text-foreground">
          {initialData ? "Edit" : "New"} Category
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="rounded-[2.5rem] border border-border bg-card p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full rounded-2xl border border-border bg-secondary/30 p-4 text-sm font-bold text-foreground focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-[2.5rem] border border-border bg-card p-8 shadow-sm">
            <label className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1 mb-4 block">Category Image</label>
